...
def _make_multinomial_logreg() -> LogisticRegression:
    """Create a multinomial LogisticRegression that works across sklearn versions.

    scikit-learn 1.8 removed the `multi_class` parameter (it was deprecated earlier).
    For older versions, we set multi_class="multinomial" explicitly.
    """
    kwargs = dict(
        solver="lbfgs",
        max_iter=5000,
...
class QuantileClipper(BaseEstimator, TransformerMixin):
    """Winsorize/clip features to reduce the impact of extreme outliers.

    The clip bounds are learned on the training split only (inside the Pipeline)
    which avoids data leakage during CV.
    """
...
def make_preprocessor(kind: str = "robust") -> Pipeline:
    ...
    robust:
      ...
      2) Quantile clipping (winsorization) at [1%, 99%]
      3) Yeo-Johnson power transform (reduces skewness / heavy tails)
...
def analysis_regression(ds: Dataset, outdir: str, test_size: float = 0.25):
    ...
    # NOTE: sklearn >= 1.8 removed `multi_class`. We use a version-proof constructor.
    logreg = Pipeline([
        ("scaler", StandardScaler()),
        ("clf", _make_multinomial_logreg()),
    ])
    logreg.fit(Xtr, ytr)
...
def analysis_validate_top_features(
    ...
    """Validate a small consensus feature set using full CV for LogReg + MLP.

    This implements the recommendation from the final slide:
      - Fix a consensus shortlist (e.g., top 8)
      - Apply robust preprocessing (outlier-resistant)
      - Re-evaluate with stratified K-fold CV (mean±std metrics)
      - Produce *stable* feature rankings:
          * Logistic regression: mean(|coef|) across folds for CKDu
          * MLP: mean permutation importance across folds (macro-F1 drop)
    """

    outdir = _ensure_outdir(outdir)

    # ---- Select the requested feature columns
    name_to_idx = {n: i for i, n in enumerate(ds.feature_names)}
    missing = [f for f in top_features if f not in name_to_idx]
    if missing:
        raise ValueError(
            f"Top feature(s) not found in dataset columns: {missing}. "
    ...
    # Persist what we actually used
    with open(os.path.join(outdir, "consensus_features_used.json"), "w") as f:
        json.dump(
            {
    ...
    # Shared preprocessor (cloned per fold)
    ...
    fold_metrics_lr = []
    coef_by_fold = []  # list of coef vectors for CKDu
    ...
    for fold, (tr, te) in enumerate(skf.split(X, y), start=1):
        ...
        # Align proba columns to labels_sorted
        classes = model.named_steps["clf"].classes_
        col_order = [int(np.where(classes == l)[0][0]) for l in labels_sorted]
        proba_aligned = proba[:, col_order]
        ...
        try:
            m["auc_macro_ovr"] = float(
                roc_auc_score(y[te], proba_aligned, multi_class="ovr", average="macro")
            )
        except Exception:
            m["auc_macro_ovr"] = float("nan")
        m["fold"] = fold
        fold_metrics_lr.append(m)

        # Coefficients for CKDu (label 1)
    ...
    lr_summary = {
        ...
        for k in ["accuracy", "f1_macro", "f1_weighted", "auc_macro_ovr"]
        if k in lr_metrics_df.columns
    }
    with open(os.path.join(outdir, "logreg_cv_summary.json"), "w") as f:
        json.dump(lr_summary, f, indent=2)
    ...
    if coef_by_fold:
        coef_mat = np.vstack(coef_by_fold)  # (folds, features)
        ...
        # Plot
        top_plot = coef_summary_df.iloc[::-1]
        fig, ax = plt.subplots(figsize=(8.2, 4.8))
        ax.barh(top_plot["feature"], top_plot["abs_coef_mean"], xerr=top_plot["abs_coef_std"])
        ax.set_xlabel("Mean |coef| across folds (± std)")
        ax.set_title("LogReg CKDu feature importance stability (top set)")
        ax.grid(True, axis="x", alpha=0.25)
        fig.tight_layout()
        fig.savefig(os.path.join(outdir, "logreg_ckdu_coef_cv_bar.png"), dpi=180)
        plt.close(fig)
    ...
    for fold, (tr, te) in enumerate(skf.split(X, y), start=1):
        ...
        # Permutation importance on the fold's held-out data
        imp = permutation_importance(
            model,
            X[te],
            y[te],
    ...
    with open(os.path.join(outdir, "mlp_cv_summary.json"), "w") as f:
    ...
    with open(os.path.join(outdir, "mlp_oof_classification_report.txt"), "w") as f:
        f.write(classification_report(y, oof_pred_mlp, target_names=label_names, digits=3))
    ...
    if imp_by_fold:
        ...
        top_plot = imp_summary_df.iloc[::-1]
        fig, ax = plt.subplots(figsize=(8.2, 4.8))
        ax.barh(top_plot["feature"], top_plot["perm_importance_mean"], xerr=top_plot["perm_importance_std"])
        ax.set_xlabel("Permutation importance (macro-F1 drop), mean ± std")
        ax.set_title("MLP feature importance stability (top set)")
        ax.grid(True, axis="x", alpha=0.25)
        fig.tight_layout()
        fig.savefig(os.path.join(outdir, "mlp_perm_importance_cv_bar.png"), dpi=180)
        plt.close(fig)

    # --------------------
    # C) Quick comparison
    # --------------------
    def pack_summary(summary: Dict[str, Dict[str, float]], model_name: str) -> List[Dict[str, float]]:
        rows = []
        for metric, stats_ in summary.items():
            rows.append({
                "model": model_name,
                "metric": metric,
                "mean": stats_.get("mean", np.nan),
                "std": stats_.get("std", np.nan),
            })
        return rows

    comp_rows = pack_summary(lr_summary, "LogReg") + pack_summary(mlp_summary, "MLP")
    comp_df = pd.DataFrame(comp_rows)
    comp_df.to_csv(os.path.join(outdir, "cv_comparison_summary.csv"), index=False)

    # --------------------
    # D) Finalize ranking
    # --------------------
    # Combine the two stability-based rankings into a single "average rank" table.
    # This is a simple, defensible way to fuse a linear (LogReg) and a non-linear (MLP)
    # view of importance.
    try:
        lr_rank_path = os.path.join(outdir, "logreg_ckdu_coef_cv_summary.csv")
        mlp_rank_path = os.path.join(outdir, "mlp_perm_importance_cv_summary.csv")
        if os.path.exists(lr_rank_path) and os.path.exists(mlp_rank_path):
            lr_rank = pd.read_csv(lr_rank_path)[["feature", "abs_coef_mean"]]
            mlp_rank = pd.read_csv(mlp_rank_path)[["feature", "perm_importance_mean"]]
            merged = lr_rank.merge(mlp_rank, on="feature", how="inner")
            merged["rank_logreg"] = merged["abs_coef_mean"].rank(ascending=False, method="average")
            merged["rank_mlp"] = merged["perm_importance_mean"].rank(ascending=False, method="average")
            merged["avg_rank"] = 0.5 * (merged["rank_logreg"] + merged["rank_mlp"])
            merged = merged.sort_values("avg_rank", ascending=True)
            merged.to_csv(os.path.join(outdir, "final_rank_fused_logreg_mlp.csv"), index=False)
    except Exception:
        pass

    print("\n[Validate Top Features] Completed.")
    print(f"[Validate Top Features] Features used: {top_features}")
    print(f"[Validate Top Features] Preprocessing: {preproc_kind}")
...
def main():
    ...
    # Options for consensus validation
    ap.add_argument(
        "--top_features",
        default="F3,F1,F7,F21,F4,F23,F25,F26",
        help="Comma-separated consensus features to validate (default is an 8-feature consensus set)",
    )
    ap.add_argument(
        "--preproc",
        default="robust",
        choices=["robust", "standard"],
        help="Preprocessing pipeline for validate_top8",
    )
    ap.add_argument("--cv_splits", type=int, default=5, help="Number of CV folds for validate_top8")
    ap.add_argument("--perm_repeats", type=int, default=10, help="Permutation repeats for MLP importance")
    args = ap.parse_args()
